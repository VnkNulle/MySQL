...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `
    SELECT 
      SUM(budget) AS budget_adjusted,
      SUM(revenue) AS revenue_adjusted
    FROM 
      movies
    WHERE
      budget IS NOT NULL
      ...
      expect(result).toEqual({
        budget_adjusted: 51221081871,
        revenue_adjusted: 139589164680,
      });
...
`;
  ...
  it("should select count of ratings left each month", async (done) => {
    const query = `
    SELECT 
