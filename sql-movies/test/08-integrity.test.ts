...
describe("Foreign Keys", () => {
  ...
  it(
    ...
    async done => {
      ...
      await db.execute(`DELETE FROM movie_genres WHERE movie_id = ${movieId}`);
      await db.execute(`DELETE FROM movie_actors WHERE movie_id = ${movieId}`);
      await db.execute(`DELETE FROM movie_directors WHERE movie_id = ${movieId}`);
      await db.execute(`DELETE FROM movie_keywords WHERE movie_id = ${movieId}`);
      await db.execute(`DELETE FROM movie_production_companies WHERE movie_id = ${movieId}`);
